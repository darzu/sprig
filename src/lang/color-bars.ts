...
export function makeAllColorBars() {
    ...
    const avgMinMax = (vs: number[]) => ({ avg: m.avg(vs), min: m.min(vs), max: m.max(vs) });
    ...
    const contClrs = range(numHues)
        .map((_, i) => {
            ...
            const cs = range(9)
                ...
                .reverse()
                .map(s =>
                    contrastClamp({ h: hue, s, l: 100 }, white, min, max, ({ h, s, l }) => clampHSL({ h, s, l: l - 1 }))
    ...
    const okSatClrs = okClrs
        .map(clr => {
            ...
            const cs = range(NUM)
                ...
                .reverse()
                .map(s =>
                    contrastClamp({ h, s, l: 100 }, white, min, max, ({ h, s, l }) => clampHSL({ h, s, l: l - 1 }))
                    // contrastClamp(toLAB({ h: hue, s, l: 50 }), toLAB(white), min, max, ({ l, a, b }) => ({ l: l - 0.1, a, b }))
                )
                .filter(c => !!c)
                .map(c => c!)
...
function mkColorDistNeighborReducer<C extends Color>(minDist: number, strat: (a: Color, b: Color) => number) {
    let last: C;
    let first: C;
...
function mkColorDistAllReducer<C extends Color>(minDist: number, strat: (a: Color, b: Color) => number) {
...
function mkColorBar(clrs: Color[], lblText: string, width = 128 * 4, height = DEFAULT_BAR_HEIGHT): SVGGElement {
