...
function topHelper(w: number, notch: boolean, fromMouth: boolean, r: number): string {
...
function bottomHelper(w: number, notch: boolean, toMouth: boolean, r: number): string {
    // if notch can't fit
...
function mkBlockSimplePath(size: V2, cornerShape: CornerShape, cornerHeight?: number): string {
...
function mkBlockFullPath(r: RenderableBlock): string {
    ...
    if (r.corner === "square") {
        ...
        let maxH = max(r.sections.map(s =>
            max(s.lines.map(l =>
                max(l.nodes.map(n => n.size.y))
            ))))
