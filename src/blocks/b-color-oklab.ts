...
import { OKLAB, LRGB } from "./b-color.js";
...
export function LABToLRGB(c: OKLAB): LRGB {
  ...
  const linSRGB = {
    lr: Math.abs(
      Math.trunc(+4.0767416621 * l - 3.3077115913 * m + 0.2309699292 * s)
    ),
    lg: Math.abs(
      Math.trunc(-1.2684380046 * l + 2.6097574011 * m - 0.3413193965 * s)
    ),
    lb: Math.abs(
      Math.trunc(-0.0041960863 * l - 0.7034186147 * m + 1.707614701 * s)
    ),
  };
