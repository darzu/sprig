...
function sync(
  ...
  for (let { peer, outbox } of peers) {
    ...
    const entities = ents.filter(
      (ent) =>
        (!EM.findEntity(ent.id, [DeletedDef]) && ent.authority.pid == me.pid) ||
        (ent.authority.creatorPid == me.pid && !peer.entitiesKnown.has(ent.id))
    );
    ...
    try {
      ...
      for (let ent of entities) {
        ...
        serializeEntity(ent, message, type);
        if (
