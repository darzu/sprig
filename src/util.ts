export function range(length: number): number[] {
    return (((new Array(length)) as any)
        .fill(null) as number[])
        .map((_, i) => i)
}
...
export function asMap<T>(ts: T[], key: (t: T) => string): { [name: string]: T } {
...
export function mapObj<V, U>(o: { [key: number]: V }, fn: (v: V) => U): { [key: number]: U }
export function mapObj<V, U>(o: { [key: string]: V }, fn: (v: V) => U): { [key: string]: U } {
    const n: any = {}
    Object.keys(o)
        .forEach(k => {
            n[k] = fn(o[k])
...
export namespace ajax {
    function getFileInternal(url: string, respType: XMLHttpRequestResponseType | undefined, callback: (status: number | null, r: XMLHttpRequest) => void) {
    ...
    function getFile(url: string, respType?: XMLHttpRequestResponseType): Promise<any> {
    ...
    export function getXml(url: string): Promise<HTMLElement> {
        return getFile(url, 'document')
            .then((xmlResp: Document) => xmlResp.documentElement)
    };
}
