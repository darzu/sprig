...
export const CannonConstructDef = EM.defineComponent("cannonConstruct", (loc, yaw, pitch) => {
    return {
...
export function registerPlayerCannonSystem(em) {
    ...
    //   "playerCannon2"
    // );
    em.registerSystem([CannonDef, RotationDef], [CameraDef], (cannons, res) => {
        for (let c of cannons) {
    ...
    em.registerSystem([CannonDef], [PhysicsTimerDef], (cannons, res) => {
        for (let c of cannons) {
    ...
    em.registerSystem([CannonDef, WorldFrameDef, InteractableDef, RotationDef], [
        DetectedEventsDef,
        MusicDef,
        InputsDef,
        PhysicsResultsDef,
        ...
        for (let c of cannons) {
            ...
            const players = (_a = res.physicsResults.collidesWith
                .get(interaction.colliderId)) === null || _a === void 0 ? void 0 : _a.map((h) => em.findEntity(h, [PlayerEntDef, AuthorityDef])).filter((p) => p && p.authority.pid === res.me.pid);
            ...
            if (c.cannon.mannedId) {
                ...
                c.cannon.pitch = clamp(c.cannon.pitch, c.cannon.minPitch, c.cannon.maxPitch);
...
export function fireFromCannon(em, cannon) {
...
export function createPlayerCannon(em, e, pid, assets) {
...
export function registerBuildCannonsSystem(em) {
    em.registerSystem([CannonConstructDef], [MeDef, AssetsDef], (cannons, res) => {
        for (let b of cannons)
            createPlayerCannon(em, b, res.me.pid, res.assets);
    }, "buildCannons");
}
