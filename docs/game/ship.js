...
export function registerShipSystems(em) {
    em.registerSystem([ShipConstructDef], [MeDef, AssetsDef], (ships, res) => {
        for (let e of ships) {
            ...
            const boatFloor = min(mc.children.map((c) => c.aabb.max[1]));
            ...
            em.addComponent(cannonL.id, CannonConstructDef, [6, 3, 5], Math.PI, cannonPitch);
    ...
    em.registerSystem([ShipDef, PositionDef], [MusicDef], (ships, res) => {
    ...
    em.registerSystem([ShipDef, LinearVelocityDef], [], (ships, res) => {
        for (let s of ships) {
            s.linearVelocity[2] = s.ship.speed;
            s.linearVelocity[1] = -0.01;
        }
    }, "shipMove");
    em.registerSystem([ShipDef, PositionDef], [TextDef], (ships, res) => {
        const score = max(ships.map((s) => s.position[2]));
        // update score
        const roundScore = Math.round(score / 10);
        res.text.setText(`${roundScore}`);
    }, "shipUI");
    em.registerSystem([ShipDef], [PhysicsResultsDef, MusicDef], (ships, res) => {
        var _a;
        for (let s of ships) {
            for (let partId of s.ship.partIds) {
                ...
                if (part) {
                    if (!part.renderable.enabled)
                        continue;
                    const bullets = (_a = res.physicsResults.collidesWith
                        .get(partId)) === null || _a === void 0 ? void 0 : _a.map((h) => em.findEntity(h, [BulletDef])).filter((h) => h && h.bullet.team === 2);
