...
import { aabbCenter, checkBroadphase, collisionPairs, copyAABB, createAABB, doesOverlap, doesTouch, getAABBFromPositions, rayHitDist, resetCollidesWithSet, } from "./broadphase.js";
...
export function registerUpdateWorldAABBs(em, s = "") {
    em.registerSystem([PhysicsStateDef, WorldFrameDef], [PhysicsTimerDef, PhysicsBroadCollidersDef], (objs, res) => {
        for (let o of objs) {
            ...
            for (let i = 0; i < o._phys.worldAABBs.length; i++) {
                ...
                const wCorners = getAABBCorners(lc.aabb).map((p) => vec3.transformMat4(p, p, o.world.transform));
...
export function registerUpdateLocalPhysicsAfterRebound(em, s = "") {
    em.registerSystem([PhysicsStateDef, WorldFrameDef, TransformDef], [PhysicsTimerDef, PhysicsResultsDef, PhysicsBroadCollidersDef], (objs, res) => {
...
export function registerPhysicsStateInit(em) {
    ...
    em.registerSystem([ColliderDef], [PhysicsBroadCollidersDef], (objs, { _physBColliders }) => {
        for (let o of objs) {
...
export function registerPhysicsContactSystems(em) {
    em.registerSystem([ColliderDef, PhysicsStateDef, WorldFrameDef], [PhysicsTimerDef, PhysicsBroadCollidersDef], (objs, res) => {
        ...
        const currColliders = objs
            .map((o) => o._phys.worldAABBs.map((cId) => res._physBColliders.colliders[cId]))
            .reduce((p, n) => [...p, ...n], []);
        ...
        while (anyMovement && itr < COLLISION_MAX_ITRS) {
            ...
            for (let [aCId, bCId] of colliderPairs) {
                ...
                if (a.collider.solid && b.collider.solid) {
                    ...
                    const rebData = computeReboundData(ac, ac.lastPos, ac.pos, bc, bc.lastPos, bc.pos, itr);
