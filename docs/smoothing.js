...
import { PositionDef, PhysicsParentDef, RotationDef, ScaleDef, } from "./physics/transform.js";
...
function updateSmoothingTargetSmoothChange(objs) {
    for (let o of objs) {
        updateLocSmoothingTarget(o.motionSmoothing.positionTarget, o.motionSmoothing.positionDiff, o.position);
        if (o.rotation) {
            updateRotSmoothingTarget(o.motionSmoothing.rotationTarget, o.motionSmoothing.rotationDiff, o.rotation);
        }
    }
}
function updateSmoothingTargetSnapChange(objs) {
    for (let o of objs) {
        ...
        if (o.rotation)
            quat.copy(o.motionSmoothing.rotationTarget, o.rotation);
    }
}
function updateSmoothingLerp(objs, resources) {
    const { physicsTimer: { period: dt }, } = resources;
    for (let o of objs) {
        // lerp position
...
export function registerUpdateSmoothingTargetSnapChange(em) {
    em.registerSystem([PositionDef, MotionSmoothingDef], [], updateSmoothingTargetSnapChange);
}
export function registerUpdateSmoothingTargetSmoothChange(em) {
    em.registerSystem([PositionDef, MotionSmoothingDef], [], updateSmoothingTargetSmoothChange);
}
...
export function registerUpdateSmoothedTransform(em) {
    em.registerSystem([WorldFrameDef, MotionSmoothingDef, PositionDef], [], (objs) => {
        for (let o of objs) {
            ...
            mat4.fromRotationTranslationScale(o.world.transform, smoothRot, vec3.add(tempVec(), o.position, o.motionSmoothing.positionDiff), ScaleDef.isOn(o) ? o.scale : vec3.set(tempVec(), 1, 1, 1));
        }
