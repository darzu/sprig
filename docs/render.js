...
export class Renderer {
    constructor(canvas, device, maxMeshes = 100, maxTrisPerMesh = 100) {
    ...
    gpuBufferWriteMeshTransform(m) {
        this.device.queue.writeBuffer(this.meshUniformBuffer, m.modelUniByteOffset, m.obj.transform().buffer);
    }
    ...
    getMappedRanges() {
        ...
        let meshUniformBuffer = new Float32Array(this.meshUniformBuffer.getMappedRange());
    ...
    checkCanvasResize() {
        if (this.lastWidth === this.canvas.width &&
    ...
    */
    addObject(o) {
        ...
        m.tri.forEach((triInd, i) => {
            ...
            if (mapped === null) {
                this.device.queue.writeBuffer(this.vertexBuffer, (vOff + 0 * vertElStride) * bytesPerFloat, new Float32Array([...m.pos[triInd[0]], ...m.colors[i], ...normal])
                    .buffer);
                this.device.queue.writeBuffer(this.vertexBuffer, (vOff + 1 * vertElStride) * bytesPerFloat, new Float32Array([...m.pos[triInd[1]], ...m.colors[i], ...normal])
                    .buffer);
                this.device.queue.writeBuffer(this.vertexBuffer, (vOff + 2 * vertElStride) * bytesPerFloat, new Float32Array([...m.pos[triInd[2]], ...m.colors[i], ...normal])
                    .buffer);
            }
            else {
                mapped.vertexBuffer.set([...m.pos[triInd[0]], ...m.colors[i], ...normal], vOff + 0 * vertElStride);
                mapped.vertexBuffer.set([...m.pos[triInd[1]], ...m.colors[i], ...normal], vOff + 1 * vertElStride);
                mapped.vertexBuffer.set([...m.pos[triInd[2]], ...m.colors[i], ...normal], vOff + 2 * vertElStride);
            }
            this.numVerts += 3;
            this.numTris += 1;
        });
        ...
        if (mapped === null) {
            this.device.queue.writeBuffer(this.meshUniformBuffer, uniOffset, o.transform().buffer);
        }
        else {
            mapped.meshUniformBuffer.set(o.transform(), uniOffset / bytesPerFloat);
        }
        const res = {
            vertNumOffset,
            indicesNumOffset,
    ...
    createRenderBundle() {
        ...
        for (let m of this.meshHandles) {
            ...
            bundleEnc.drawIndexed(m.triCount * indicesPerTriangle, undefined, m.indicesNumOffset, m.vertNumOffset);
        }
    ...
    setupSceneBuffer() {
        ...
        this.device.queue.writeBuffer(this.sceneUniformBuffer, bytesPerMat4 * 1, lightViewProjMatrix.buffer);
        this.device.queue.writeBuffer(this.sceneUniformBuffer, bytesPerMat4 * 2, lightDir.buffer);
    }
