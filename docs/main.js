...
class Player extends Cube {
    constructor(id, creator, playerId) {
        super(id, creator);
...
class CubeGameState extends GameState {
    constructor(time, renderer, createObjects = true) {
        super(time, renderer);
        this.time = 0;
        this.me = 0;
        ...
        if (createObjects) {
            ...
            for (let i = 0; i < 200; i++) {
                ...
                cube.location = vec3.fromValues(Math.random() * 20 - 10, Math.random() * 5, -Math.random() * 10 - 5);
                ...
                cube.angular_velocity = vec3.scale(vec3.create(), cube.axis, Math.PI * 0.001);
                cube.at_rest = false;
    ...
    stepGame(dt, inputs) {
        ...
        for (let cube of this.cubes) {
            ...
            for (let player of Object.values(this.players)) {
                let cube_to_player = vec3.subtract(vec3.create(), player.location, cube.location);
                let distance = vec3.length(cube_to_player);
                if (distance < min_distance) {
                    min_distance = distance;
                    min_distance_player = player.authority;
                }
                if (vec3.length(cube_to_player) < 8 &&
